...
try {
  ...
  assert.equal(moving.gait, 1); assert.ok(moving.z < before.z - 1);
  await page.keyboard.down('ArrowUp'); await page.waitForTimeout(350); await page.keyboard.up('ArrowUp');
  assert.equal((await page.evaluate(() => window.__polana.snapshot())).gait, 2);
  await page.keyboard.press('Space'); await page.waitForTimeout(300);
  assert.ok((await page.evaluate(() => window.__polana.snapshot())).height > 0.5);
  await page.keyboard.press('KeyC');
  assert.equal((await page.evaluate(() => window.__polana.snapshot())).firstPerson, true);
  await page.screenshot({ path: 'artifacts/first-person.png' });
  await page.keyboard.press('Escape');
  const paused = await page.evaluate(() => window.__polana.snapshot());
  assert.equal(paused.paused, true); await page.waitForTimeout(250);
  assert.equal((await page.evaluate(() => window.__polana.snapshot())).z, paused.z);
  await page.getByRole('button', { name: 'Wracamy do jazdy', exact: true }).click();
  ...
  const previewPosition = await page.evaluate(() => window.__polana.snapshot());
  const previewCanvas = page.locator('#horse-preview');
  const originalPreview = await previewCanvas.screenshot();
  await page.getByRole('button', { name: 'Obróć w prawo', exact: true }).click();
  await page.getByRole('button', { name: 'Obróć w prawo', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Pokaż jeźdźca', exact: true }).click();
  assert.equal(await page.getByRole('button', { name: 'Pokaż jeźdźca', exact: true }).getAttribute('aria-pressed'), 'true');
  ...
  await page.getByRole('button', { name: 'Ozdoba: Kwiaty', exact: true }).click();
  await page.screenshot({ path: 'artifacts/wardrobe.png' });
  assert.equal((await page.evaluate(() => window.__polana.snapshot())).z, previewPosition.z);
  await page.getByRole('button', { name: 'Wracamy na polanę', exact: true }).click();
  await page.reload(); await page.waitForFunction(() => window.__polana?.snapshot().calls > 0);
  await page.getByRole('button', { name: 'Wygląd konia', exact: true }).click();
  await page.getByRole('button', { name: 'Kolory', exact: true }).click();
  assert.equal(await page.getByRole('button', { name: 'Maść: Siwa', exact: true }).getAttribute('aria-pressed'), 'true');
  await page.getByRole('button', { name: 'Wracamy na polanę', exact: true }).click();
  await page.getByRole('button', { name: 'Do stajni', exact: true }).click();
  ...
  assert.equal((await page.evaluate(() => window.__polana.snapshot())).obstacles[0].down, 0);
  await page.waitForFunction(() => window.__polana.snapshot().z < -9);
  assert.ok((await page.evaluate(() => window.__polana.snapshot())).obstacles[1].down > 0);
  ...
  assert.equal((await page.evaluate(() => window.__polana.snapshot())).gait, 3);
  ...
  assert.equal(await page.locator('#dress-dialog').evaluate(dialog => dialog.scrollWidth <= dialog.clientWidth), true);
  await page.keyboard.press('Escape');
  assert.equal((await page.evaluate(() => window.__polana.snapshot())).paused, false);
  console.log('Browser checks passed: rendering, persistent gait, jump, cameras, pause, reset, appearance persistence, successful rail clearance, missed rail, 640px and 320px layouts; no page errors.');
