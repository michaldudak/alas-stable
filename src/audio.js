...
export class Soundscape {
  ...
  tone(frequency, duration, volume, endFrequency, type = 'sine') {
    if (!this.enabled || this.context?.state !== 'running') return;
    const ctx = this.context, oscillator = ctx.createOscillator(), gain = ctx.createGain();
    oscillator.type = type;
    oscillator.frequency.setValueAtTime(frequency, ctx.currentTime);
    oscillator.frequency.exponentialRampToValueAtTime(Math.max(1, endFrequency || frequency), ctx.currentTime + duration);
  ...
  tick(dt, state, sand, footfalls = 0) {
    ...
    if (this.windTime <= 0 && this.enabled && this.context?.state === 'running') {
      ...
      for (let i = 0; i < data.length; i++) data[i] = (Math.random() * 2 - 1) * 0.015 * Math.sin(Math.PI * i / data.length);
      const source = ctx.createBufferSource(), filter = ctx.createBiquadFilter();
      source.buffer = buffer; filter.type = 'lowpass'; filter.frequency.value = state.z < -40 ? 1400 : 650;
