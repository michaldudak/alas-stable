...
export const STABLE_WALLS: (Solid & { h: number; stall?: boolean })[] = [
	{ x: -12, z: 0, w: 0.4, d: 26, h: 6 },
	{ x: 12, z: 0, w: 0.4, d: 26, h: 6 },
	...[-13, 13].flatMap((z) =>
...
export function insideStable(x: number, z: number, margin = 0) {
	return (
		Math.abs(x - STABLE.x) < STABLE.halfWidth + margin &&
		Math.abs(z - STABLE.z) < STABLE.halfDepth + margin
	);
}
export function inTackRoom(x: number, z: number) {
	return insideStable(x, z) && x > STABLE.x + 3.7 && z > STABLE.z + 3;
}
